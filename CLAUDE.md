...
| `src/js/oa-legal-toc.js` | Legal pages' section index. Builds the "On this page" list from the shallowest heading level present inside `[data-legal-toc-content]` (a paste into a Webflow Rich Text does not reliably land on `h2` — Delivery & Returns came through as `h5`) by cloning the one `[data-legal-toc-link]` template. The heading's section number goes into `[data-legal-toc-prefix]` and the rest into `[data-legal-toc-text]` (both spans in the template; no prefix element keeps the whole heading in the text). The selected section is the link last taken, not the section in view — `data-legal-toc-status="active"`, set on click and from the URL hash on load, held until another link is taken. A fixed back-to-top button `[data-legal-toc-top]` (Link Block to `#`, so it works without the script) shows only while the index is folded and scrolled out of view above; the script writes `data-legal-toc-top-state` and `inert`, and hands focus to the index toggle after the scroll. Hooks are `[data-legal-toc-list]` (holds the one template link) and `[data-legal-toc-content]` (the Rich Text — optional: without it the script takes the page's `.w-richtext` with the most headings, because pasting a new document into a duplicated page's Rich Text drops its custom attributes — and writes the attribute back, since the CSS landing offset hangs off it), resolved page-wide — they do not have to share a wrapper, because the Designer layout puts them in sibling branches of the `• oa Titles + Text` component. Adapted from Osmo's Table of Contents; their click handler was dropped because `initNavAnchorLinks` in `oa-global.js` already owns same-page anchor scrolling — the element holding the index links opts in with `data-oa-anchor-scroll`. Section numbers are stripped from the generated ids (a digit-leading id is a valid id but an invalid CSS selector, and `initNavAnchorLinks` resolves the target with `querySelector`). Below the Lumos medium boundary the index folds into an accordion this file owns by attribute: `[data-legal-toc-accordion]` (the `.oa-accordion_item`; carries the timing knobs `data-legal-toc-duration` / `-ease` / `-duration-close` / `-ease-close` / `-stagger`), `[data-legal-toc-toggle]` (the button; its `aria-expanded` is the state), `[data-legal-toc-panel]` (height animates), `[data-legal-toc-label]` (desktop-only label). The open look is Designer work through Lumos's `data-state="expanded"` → `--_state---true/false`. A link tap closes it instantly from a `window` capture click listener — ahead of `initNavAnchorLinks`, and never on `pointerdown`, which moves the link out from under the tap. GSAP is used for the accordion motion only; without it the index still builds, navigates and toggles. | Raw file → CDN |
| `src/css/oa-legal-toc.css` | Behavioural glue for `oa-legal-toc.js`: the heading `scroll-margin-top` (nav clearance, same 130px as `oa-all-products.css`), the active-state rule on `--swatch--brand-500`, the empty-prefix hide (an unnumbered heading such as Preamble starts on the number column instead of one flex gap in), the accordion's show/hide states inside `@container (width < 50em)` (Lumos's medium boundary), the back-to-top hidden state (opacity + pointer-events), and the `wf-design-mode` preview. Grid split, sticky offset (8.125rem, to match the 130px landing), gaps, type, colours and the back-to-top button's look, position and fade are Designer knobs. | Raw file → CDN |
...
ScrollTrigger consumers (e.g. `oa-text-reveal.js`) run through the Lenis↔ScrollTrigger glue in `oa-global.js` (`lenis.on('scroll', ScrollTrigger.update)`) — verify scroll-triggered starts against Lenis, not native scroll. Each must load after `oa-global.js` so Lenis exists when its triggers are created.
...
- `oa-legal-toc.js` + `oa-legal-toc.css` — the legal pages (`/legal/*`). Page-level embed, after the sitewide footer so `oa-global.js` has created Lenis and the anchor handler first.
