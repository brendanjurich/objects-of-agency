...
## 2026-06-28 — Nav swap: Osmo Multilevel Nav replaces the Webflow-native nav

Replaced the minimal Webflow Navbar (`.nav_component` + `.w-nav-button`, `initNavSafariFix()`, mobile `mix-blend-mode:difference`) with **Osmo Supply's Multilevel Navigation** (class `.nav`, `data-` attribute driven). CSS ported into `oa-styles.css`, JS into `oa-global.js` (section 7, replacing the Safari nav fix). Zero class-name collisions except `body.menu-open` (deliberately reused for scroll-lock). Decisions made in a grill session:

- **em→rem is selective, not blanket.** Osmo uses `em` as a *scaling system* — `--nav-bg-height: calc(20em + (2em+3em+2.5em+3em))` is computed from the same em paddings used on `.nav-inner`/`.nav-container`/`.nav-dropdown`/`.nav-dropdown__link`. Converting blindly would desync the calc from its parts. Rule applied: **font-size multipliers stay `em`** (`.nav-link__label` 1.25em, `.nav-dropdown__link-label` 1.75em, the dropdown-icon + bubble sizing); **structural box-metrics + the height calc convert to `rem` as a coherent set**. At default zoom the rem values equal the em they replaced (the nav sets no custom root font-size), so the swap locks out type-scale drift without moving the layout.
- **svh, not dvh/vh**, for the mobile menu cover (`--nav-bg-height: 100svh`, `.nav-center: 100svh`) and `.page-bg` overlay. Scroll is locked while the menu is open, so dvh's dynamic resize buys nothing; svh honours the project's svh-over-dvh rule and the list scrolls internally so nothing is cut.
- **Dropped `mix-blend-mode`** in favour of Osmo's explicit white→dark colour flip (white nav over hero → dark text on the expanding white panel + page dim). This **retires the old `autoAlpha`/`visibility` ban** on the nav (it existed only to protect the blend mode). Caveat carried forward: white closed-state nav text assumes a dark-enough hero — verify per page.
- **Easing unified + renamed.** Osmo's `--cubic-default: cubic-bezier(0.525,0,0,1)` is a lazy-start curve — exactly what REFERENCE.md rejects as "reads as lag on touch." Pointed the nav's `--cubic-default` at our site curve, and **renamed `--ease-osmo` → `--ease-oa` project-wide** to signal ownership (same value, 5 usages in `oa-styles.css` + REFERENCE.md). Kept Osmo's `--duration-fast/normal`.
- **Init at execution, not `DOMContentLoaded`.** Osmo inits on `DOMContentLoaded`, which `hls.js` holds back ~17s on Slow 4G — that would freeze the mobile burger. `oa-global.js` is a footer script so the nav markup is already parsed; `initNavigation()` now runs at top-level execution. Desktop dropdowns are pure-CSS hover so they're unaffected either way.
- **Global resets + vars scoped to `.nav`.** Osmo's bare `a {}` / `*:focus-visible {}` resets and `:root` vars would bleed site-wide and clobber Lumos link/focus styling. Scoped all of it under `.nav`; palette ported as tunable nav-scoped vars (`--nav-color-dark`, `--nav-panel`, `--nav-panel-hover`, `--nav-bubble`) for Designer retuning.
- **Mobile-toggle wiring.** The burger click drives `window.lenis.stop()/start()` + `body.menu-open` (parity with the retired nav fix). Desktop dropdowns do **not** lock scroll.

Structure is built in the Webflow Designer (the `<nav data-menu-status>` tree + `data-` attributes + static dropdown cards — no CMS binding). Open follow-up: re-check the `scroll-margin-top: 130px` anchor offset in `oa-all-products.css` against the new nav's real fixed height once it's on staging.
